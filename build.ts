...
const copyFiles = async (patterns: string[], srcDir: string, destDir: string): Promise<{ path: string; size: number }[]> => {
...
if (existsSync('./recipe_manager')) {
  ...
  await rm('./recipe_manager', { recursive: true, force: true });
}
...
const copiedFiles = await copyFiles(["**/*.tsx", "**/*.ts", "**/*.svg"], "src", outdir);
...
console.log(`📦 Total files: ${allOutputs.length} (${result.outputs.length} built + ${copiedFiles.length} copied)\n`);
